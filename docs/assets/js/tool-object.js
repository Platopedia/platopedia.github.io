...
function _Tool_Object ( param )
{
    ...
    that.wait = function ( cond, callback )
    {
        const poll = resolve =>
        {
            if   ( cond( ) ) resolve( );
        ...
        return new Promise( poll ).then( callback );
    };
...
function Tool_Object_Cropper ( param )
{
    var that = _util_object
    (
        _Tool_Object,
