...
function on_tableb_groups ( ) {
  ...
  //api.draw( );
    ...
    api.column( 1 ).data( ).unique( ).sort( ).each( function ( type ) {
                
        filter_type.append( '<option value="1=' + type + '">' + type + '</option>' );
...
function on_tableb_concepts ( ) {
  ...
  //api.draw( );
    ...
    $( '\
        ...
        <br />\
        <label>\
            <select class="filter-author custom-select custom-select-sm form-control form-control-sm">\
                <option value="" selected>Any Author</option>\
            </select>\
        </label>\
        <br />\
    ' )
    ...
    var filter_type   = filter.find( '.filter-type' );
    var filter_author = filter.find( '.filter-author' );
    var filters       = $( [ filter_type, filter_author ] );
    ...
    api.column( 2 ).data( ).unique( ).sort( ).each( function ( type ) {
                
        filter_author.append( '<option value="2=' + type + '">' + type + '</option>' );
