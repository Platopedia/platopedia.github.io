...
function on_ready ( ) {
    ...
    content_collect.each( function ( ) {
        ...
        var on = on_collect;
        if ( element.hasClass( 'content-collect-groups'   ) ) on = on_collect_filter;
        if ( element.hasClass( 'content-collect-concepts' ) ) on = on_collect_filter;
...
function on_table_items ( ) {
  ...
  //api.draw( );
    ...
    api.column( 0 ).data( ).unique( ).sort( sort_lowercase ).each( function ( type ) {
...
function on_collect_filter ( ) {
  ...
  //api.draw( );
    ...
    if ( container.parent( ).hasClass( 'content-collect-groups' ) ) {
...
function sort_uppercase (textA,textB) { return textA.toUpperCase().localeCompare(textB.toUpperCase()) }
