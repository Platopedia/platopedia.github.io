...
function on_table_items ( ) {
    ...
    var menu = $.map
    (
        _menu( api, table.find( 'thead tr' ), table.find( 'thead tr th' ) ),
    ...
    api.columns( [ 0, 1, 2, 3 ] ).visible( true  );
...
function on_load ( ) {
    ...
    if ( --content_count === -1 ) {
        ...
        $( '.loaded-i'  ).attr( 'style', 'display:inline !important'       );
        $( '.loaded-b'  ).attr( 'style', 'display:block !important'        );
        $( '.loaded-ib' ).attr( 'style', 'display:inline-block !important' );
        
    }
    
}

function _menu ( api, row, cols ) {
        ...
        cols.each( function ( col_idx ) { var col = $( this );
            ...
            $( col_conf.menus ).each( function ( ) {
                ...
                if ( row_type == 'bool' ) {
                    ...
                    api.column( col_idx ).data( ).unique( ).sort( order_lowercase ).each( function ( val ) {
...
function order_lowercase (textA,textB) { return textA.toLowerCase().localeCompare(textB.toLowerCase()) }
function order_uppercase (textA,textB) { return textA.toUpperCase().localeCompare(textB.toUpperCase()) }
