...
function Tool_Profilebuilder ( param )
{
    ...
    (
        {
            ...
            'callback' :
            {
                'click' : function ( event )
                {
                  ...
                  //picture_element.width = 1000;
                    var url = picture_element.toDataURL( 'image/png' );
                    var name = 'profilebuilder_' + Date.now( ) + '.png';
                    _util_download( url, name );
                },
            },
        }
    );
    ...
    (
        {
            ...
            'rebuild'  : true,
            'validate' : function ( value ) { return value.match( /platopedia/i ) },
        }
    );
    ...
    (
        {
            ...
            'callback' : 
            {
                ...
                {
                    var that = event.data.that;
                    _canvas_chat( undefined, that.element.find( 'canvas' ), url, 400, 100 );
                },
            },
        }
    );
                ...
                'init click' : function ( event )
                {
                    ...
                    (
                        ...
                        function ( )
                        {
                            ...
                            _canvas( undefined, canvas_default, picture, platoid, bio, banner, frame, frame_layer, chat, chat_color, chatbadge, badge, starbadge, starbadge_n );
                        },
                    );
                },
            },
    ...
    function _valid ( )
    {
        var valid_platoid = form_default_field_platoid_input_default.validate( );
        var valid_bio     = form_default_field_bio_input_default    .validate( );
        var valid         = valid_platoid && valid_bio;
        
        form_default_field_picture_input_default.element.removeClass( 'is-invalid' ); // quickfix
        
        return valid;
    }
    
    function _canvas ( deferred, canvas, picture, platoid, bio, banner, frame, frame_layer, chat, chat_color, chatbadge, badge, starbadge, starbadge_n )
    {
        ...
        frame_element.src = frame;
        
        var chat_deferred = $.Deferred( );
        var chat_element = document.createElement( 'canvas' );
        _canvas_chat( chat_deferred, chat_element, chat, 900, 100 );
        ...
        chatbadge_element.src = chatbadge;
        ...
        starbadge_element.src = starbadge;
        ...
        $.when( banner_deferred, frame_deferred, chat_deferred, chatbadge_deferred, badge_deferred, starbadge_deferred ).then
        (
            function ( )
            {
                canvas.innerHTML = '';
    ...
    function _canvas_chat ( deferred, canvas, chat, repeat_x = 0, repeat_y = 0, divide = 0 )
    {
        if ( canvas instanceof jQuery ) canvas = canvas.get( 0 );
